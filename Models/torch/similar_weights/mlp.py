...
def mlp_classifier(x_train, y_train,
                     config, # [lr, batch, reg, reg_lam, hidden, sim_lam]
                     random_seed,
                     method,
                     event_idx, val_idx,
                     epoch='dynamic',
    ...
    if method == "multi":
        ...
        mul_decoder = Decoder(hidden_layers+[len(event_idx)], True)
        mul_optimizer = torch.optim.Adam(list(mul_encoder.parameters())+list(mul_decoder.parameters()),
                                        lr=learning_rate)
        pos_weights = torch.tensor(y_train.sum(axis=0)/y_train.sum())
        
        for iter in range (max_epochs):
            train_loss = 0
            for X, Y in train_batch:
                batch_loss = nn.functional.binary_cross_entropy(
                    mul_decoder(mul_encoder(X)), Y[:,event_idx],
                    reduction='sum',
                )
                regularization_loss = 0
                if reg == 'L1':
                ...
                param_sim_loss=0
                coef, bias = list(mul_decoder.parameters())
